...
public class TransactionSettings {
     ...
     */
    public TransactionSettings(Context context, String apnName, int phoneId) {
    	mPhoneId = phoneId;
        ...
        cursor = SqliteWrapper.query(context, context.getContentResolver(),
                            Uri.withAppendedPath(Telephony.Carriers.getContentUri(mPhoneId), "current"),
        ...
        try {
            while (cursor.moveToNext() && TextUtils.isEmpty(mServiceCenter)) {
                ...
                if (isValidApnType(cursor.getString(COLUMN_TYPE), Phone.APN_TYPE_MMS)) {
                    ...
                    String servicesUri = cursor.getString(COLUMN_MMSC);
                    if(!TextUtils.isEmpty(servicesUri)){
                      mServiceCenter = servicesUri.trim();  
                    }
     ...
     * false otherwise.
     */
    public TransactionSettings(String mmscUrl, String proxyAddr, int proxyPort, int phoneId) {
