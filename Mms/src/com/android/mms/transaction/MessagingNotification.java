...
public class MessagingNotification {
    ...
    private static final int WAKETIME = 10000;//The lock will be released after the given timeout in milliseconds.

    private static final int NOTIFICATION_ID = 123;
    public static final int MESSAGE_FAILED_NOTIFICATION_ID = 789;
    public static final int DOWNLOAD_FAILED_NOTIFICATION_ID = 531;
    private final static String START_TONE_DEV_PATH = "/dev/pipe/ipc.0.0";
    private final static String END_TONE_DEV_PATH = "/dev/pipe/ipc.0.1";
    private final static char TONE_DEV_SET_VALUE = '1';
    ...
    public static void blockingUpdateNewMessageIndicator(Context context, boolean isNew,
        ...
        /********************begin*******************/
        //if is new message and count > 0,send a broadcast to update the inBox messages count
        if(count > 0 && isNew )
        {
             final String action = "android.provide.NEW_MESSAGE_INSERT";
       ...
       /*********************end********************/
        ...
        // Move this code segment to Class "MessageStatusReceiver"
    ...
    private static final MmsSmsDeliveryInfo getSmsNewDeliveryInfo(Context context) {
        ...
        try {
            ...
            Log.d(TAG, "getSmsNewDeliveryInfo: address = " + address);
    ...
    public static void classZeroNotification(Context context) {
        Notification notification = new Notification();
        SharedPreferences notificationPreference = PreferenceManager
                .getDefaultSharedPreferences(context);
        String vibrateWhen;
        if (notificationPreference
                .contains(MessagingPreferenceActivity.NOTIFICATION_VIBRATE_WHEN)) {
            vibrateWhen = notificationPreference
                    .getString(
            ...
            vibrateWhen = notificationPreference.getBoolean(
                    MessagingPreferenceActivity.NOTIFICATION_VIBRATE, false) ? context
                    .getString(R.string.prefDefault_vibrate_true) : context
        ...
        boolean nowSilent = audioManager.getRingerMode() == AudioManager.RINGER_MODE_SILENT;

        String ringtoneStr = notificationPreference.getString(
        ...
        notification.sound = TextUtils.isEmpty(ringtoneStr) ? null : Uri
                .parse(ringtoneStr);
        if (vibrateAlways || (messageSilent && vibrateSilent && !nowSilent)) {
            notification.defaults |= Notification.DEFAULT_VIBRATE;
        }
        if (TelephonyManager.getDefault().getCallState() != TelephonyManager.CALL_STATE_IDLE) {
            Vibrator callVibrator = (Vibrator) context
                    .getSystemService(Context.VIBRATOR_SERVICE);
            callVibrator.vibrate(200);
        }
        ...
        WakeLock wl = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK
                | PowerManager.ACQUIRE_CAUSES_WAKEUP, "MessagingNotification");
        wl.acquire(WAKETIME);
    }
...
//          boolean nowSilent = audioManager.getRingerMode() == AudioManager.RINGER_MODE_VIBRATE;
            boolean nowSilent = audioManager.getStreamVolume(AudioSystem.STREAM_RING) == 0;
            //===== fixed CR<NEWMS00121394,NEWMS00121342> by luning at 11-10-09 end ======

            String ringtoneStr = sp.getString(MessagingPreferenceActivity.NOTIFICATION_RINGTONE,
                    null);
            boolean messageSilent = TextUtils.isEmpty(ringtoneStr);//for bug 456
            if(!isCallstateRing()){
            ...
            if(isCallstateOffhock()){
                new Thread(){
                    @Override
                    public void run() {
                        FileOutputStream startOutputstream = null;
                        FileOutputStream endOutputstream = null;
                        try {
                            startOutputstream = new FileOutputStream(START_TONE_DEV_PATH);
                            startOutputstream.write(TONE_DEV_SET_VALUE);
                            startOutputstream.close();
                            startOutputstream = null;
                            Thread.sleep(4000);
                            endOutputstream = new FileOutputStream(END_TONE_DEV_PATH);
                            endOutputstream.write(TONE_DEV_SET_VALUE);
                            endOutputstream.close();
                            endOutputstream = null;
                        } catch (IOException e) {
                            Log.e(TAG,"Open call waiting tone failed.",e);
                            ...
                            if(startOutputstream != null){
                                try {
                                    ...
                                    Log.e(TAG,"Close call waiting tone Dev failed.",e);
                                }
                             }
...
//                Vibrator callVibrator = (Vibrator)context.getSystemService(Context.VIBRATOR_SERVICE);
//                callVibrator.vibrate(200);
            }
            Log.d(TAG,"vibrateAlways:"+vibrateAlways+",vibrateSilent:"+vibrateSilent+",nowSilent:"+nowSilent);
        ...
        if(isNew){
            Log.d(TAG,"NotificationManager dealwith notification info:========>notification.flags:"+notification.flags+",notification.defaults:"+notification.defaults+",notification.deleteIntent:"+notification.deleteIntent+",notification.sound:"+notification.sound);
        }
        nm.notify(NOTIFICATION_ID, notification);

        //wake the screen,when the message received.TS add.
        PowerManager pm =  (PowerManager) context.getSystemService(Context.POWER_SERVICE);
        WakeLock wl = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "MessagingNotification");
        wl.acquire(WAKETIME);
    }
    ...
    public static void notifyDownloadFailed(Context context, long threadId,boolean noisy) {
        notifyFailed(context, true, threadId, noisy);
    }
    ...
    private static boolean isCallstateNotIdle() {
    ...
    private static boolean isCallstateRing() {
        if(TelephonyManager.getPhoneCount() > 1){
            int callState0 = TelephonyManager.getDefault(0).getCallState();
            int callState1 = TelephonyManager.getDefault(1).getCallState();
            if (callState0 == TelephonyManager.CALL_STATE_RINGING
                    || callState1 == TelephonyManager.CALL_STATE_RINGING) {
                return true;
            } else {
                return false;
            }
    ...
    private static boolean isCallstateOffhock() {
        if(TelephonyManager.getPhoneCount() > 1){
            int callState0 = TelephonyManager.getDefault(0).getCallState();
            int callState1 = TelephonyManager.getDefault(1).getCallState();
            if (callState0 == TelephonyManager.CALL_STATE_OFFHOOK
                    || callState1 == TelephonyManager.CALL_STATE_OFFHOOK) {
                return true;
            } else {
                return false;
            }
