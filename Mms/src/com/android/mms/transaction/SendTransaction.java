...
public class SendTransaction extends Transaction implements Runnable {
    ...
    public SendTransaction(Context context,
            int transId, TransactionSettings connectionSettings, String uri, int phoneId) {
        super(context, transId, connectionSettings, phoneId);
        mSendReqURI = Uri.parse(uri);
        mId = uri;

        // Attach the transaction to the instance of RetryScheduler.
        attach(RetrySchedulerProxy.getInstance(context, mPhoneId));
    }
...
//            }
            Log.d(TAG, "don't set lineNumber");
            ...
            SendConf conf = (SendConf) new PduParser(response).parse(mPhoneId);
            if (conf == null) {
            ...
            /* fixed CR<NEWMS00148502> by luning at 2011.12.8 begin*/
            else{
                Intent notify = new Intent(PushReceiver.NOTIFY_SHOW_MMS_REPORT_ACTION);
                notify.putExtra("report", mContext.getString(R.string.mms_send_success));
                mContext.sendBroadcast(notify);
            }
            /* fixed CR<NEWMS00148502> by luning at 2011.12.8 end*/
