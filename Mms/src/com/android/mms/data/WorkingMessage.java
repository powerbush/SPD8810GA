...
import com.android.mms.transaction.FDNUtils;
...
public class WorkingMessage {
    ...
    public static final int SLIDE_MAX = -5;
    ...
    public static final int OTHER_FILE = 6;/*fixed CR<NEWMS00144166> by luning at 2011.11.28 end*/
    ...
    // ======modify by phone2 at 12-02-15 begin======
    public static boolean fdnEnable;
    // ======modify by phone2 at 12-02-15 end======
     ...
     */
    public static WorkingMessage load(ComposeMessageActivity activity, Uri uri, boolean notify) {
    ...
    private boolean loadFromUri(Uri uri, boolean notify) {
        if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) LogTag.debug("loadFromUri %s", uri);
        try {
            mSlideshow = SlideshowModel.createFromMessageUri(mActivity, uri, false);
        } catch (MmsException e) {
        ...
        if (mSlideshow.hasVcard()) {
            VcardModel vcardModel = mSlideshow.mVcards.get(0);
            mSlideshow.increaseMessageSize(vcardModel.getMediaSize());
            ...
            FileModel fileModel = mSlideshow.mFiles.get(0);
            mSlideshow.increaseMessageSize(fileModel.getMediaSize());
            if (mSlideshow.size() > 1) {
                mAttachmentType = SLIDESHOW;
            } else {
                mAttachmentType = OTHER_FILE;
    ...
    private boolean loadFromConversation(Conversation conv) {
        if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) Log.d(TAG,"loadFromConversation "+ conv);
    ...
    private void changeMedia(int type, Uri uri) throws MmsException {
        ...
        if (type == IMAGE) {
            ...
            String src = (String)mContext.getResources().getText(R.string.attach_vcard);
//	    attach_vcard
            // if (path != null && !"".equals(path)) {
            //     src = path.substring(path.lastIndexOf('/') + 1);
            //     src = System.currentTimeMillis()+"_"+src;
            // }
            media = new VcardModel(mActivity, ContentType.TEXT_VCARD, src, uri);
        } else if (type == OTHER_FILE) {
            String uriStr = uri.toString();
            String[] str = uriStr.split("/");
            int num = str.length > 0 ? (str.length - 1) : 0;

            media = new FileModel(mContext, SmilHelper.ELEMENT_TAG_FILE,
                    ContentType.FILE_UNSPECIFIED, str[num], uri);
        } else {
    ...
    public void setSubject(CharSequence s, boolean notify) {
        ...
        try{
	        if(s!=null) {
	            int sublen = s.toString().getBytes().length;
	            if (requiresMms()) {
	                ensureSlideshow();
	                mSubjectLength = sublen;
	                mSlideshow.setSubjectSize(sublen);
	            }
	        }
		} catch (ExceedMessageSizeException e) {
		}
    ...
    public Uri saveAsMms(boolean notify) {
        if (DEBUG) Log.d(TAG,"save mConversation=" + mConversation);
    ...
    public void saveDraft() {
        ...
        Log.d(TAG,"saveDraft() mDiscarded:"+mDiscarded);
        if (mDiscarded) {
            return;
        }
        ...
        if (LogTag.VERBOSE || DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            Log.d(TAG,"saveDraft for mConversation:" + mConversation);
        }
    ...
    public void setConversation(Conversation conv) {
        if (DEBUG) Log.d(TAG,"setConversation " + mConversation + " -> " + conv);
    ...
    */
    ...
    private void updateState(int state, boolean on, boolean notify) {
        ...
        if (oldState != mMmsState) {
            if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) LogTag.debug("updateState: %s%s = %s",
                    on ? "+" : "-",
    ...
    public boolean send(final String recipientsInUI) {
        ...
        // ======modify by phone2 at 12-02-15 begin======
        //SystemProperties.getBoolean("gsm.sim.fdn.enable", false);
        fdnEnable = TelephonyManager.getDefault(mPhoneId).getIccFdnEnabled();
        if (fdnEnable) {
            FDNUtils.getInstance(mPhoneId).queryFDN(mActivity);
        }
        // ======modify by phone2 at 12-02-15 end======
        ...
        if (requiresMms() || addressContainsEmailToMms(conv, msgTxt)) {
            ...
            if (fdnEnable) {
                ...
                contactList = checkFDN(contactList,mPhoneId);
                if (contactList.size() > 0) {
                    conv.setRecipients(contactList);
                } else {
                    return false;
                }
            }
            ...
            if ((slideshow.size() > 0 && slideshow.get(0).hasVcard())) {
                if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                    Log.d(TAG, "mms send vcard");
                }

                new Thread(run).start();
            } else {
                if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                    Log.d(TAG, "mms send not vcard");
                }
            ...
            // ======modify by phone2 at 2012-02-15 begin======fix for bug 10630
            if (fdnEnable && FDNUtils.getInstance(mPhoneId).isSmscDisable()) {
                Toast.makeText(
                        mContext,
                        mContext.getString(R.string.send_need_smsc), Toast.LENGTH_SHORT).show();
                return false;
            }
            if (fdnEnable) {
                ContactList contactList = conv.getRecipients();
    ...
    private void sendSmsWorker(final String msgText, String semiSepRecipients, final long threadId) {
        ...
        if (LogTag.VERBOSE || DEBUG || Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.d(TAG,"sendSmsWorker sending message: recipients=" + semiSepRecipients +
		  ", threadId=" + threadId);
        }

	MessageSender sender = new SmsMessageSender(mActivity, dests, msgText, threadId,mPhoneId);
	try {
    ...
    private void sendMmsWorker(Conversation conv, Uri mmsUri, PduPersister persister,
        ...
        MessageSender sender = new MmsMessageSender(mActivity, mmsUri,
						    slideshow.getCurrentMessageSize(),mPhoneId);
        try {
    ...
    private void asyncUpdateDraftMmsMessage(final Conversation conv) {
        if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            Log.d(TAG,"asyncUpdateDraftMmsMessage conv="+conv+" mMessageUri="+ mMessageUri);
        }
    ...
    private String readDraftSmsMessage(Conversation conv) {
        ...
        if (haveDraft && conv.getMessageCount() == 0) {
            ...
            // the activity.
        	//Lino modify for NEWMS00134122 begin 2011-11-03
            //clearConversation(conv);
        	//Lino modify for NEWMS00134122 end 2011-11-03
        }
    ...
    private void clearConversation(final Conversation conv) {
        asyncDeleteDraftSmsMessage(conv);
        conv.setDraftState(false);/*add for CR<NEWMS00145575> by luning at 2011.12.10*/
        if (conv.getMessageCount() == 0) {
            if (DEBUG) LogTag.debug("clearConversation calling clearThreadId");
            conv.clearThreadId();
        }

//        conv.setDraftState(false);/*delete for CR<NEWMS00145575> by luning at 2011.12.10*/
    }
    ...
    private void updateDraftSmsMessage(long thread_id, String contents) {
        if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            Log.d(TAG,"updateDraftSmsMessage tid="+thread_id+", contents="+contents);
        }
    ...
    // ======fixed CR<NEWMS00110179> by luning at 11-08-12 begin======
    public int addAttachment(int type, Uri dataUri){
        if (DEBUG || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            LogTag.debug("addAttachment type=%d uri %s", type, dataUri);
        }
        int result = OK;
        // Make sure mSlideshow is set up and has a slide.
        ensureSlideshow();
        ...
        MediaModel media ;
		try {
			for (SlideModel slide : mSlideshow) {
				if (type == IMAGE && !slide.hasImage() && !slide.hasVideo() && !slide.hasVcard() 
				        && !slide.hasOtherFile()) {
					isSlideAdd = true;
					media = new ImageModel(mContext, dataUri, mSlideshow
							.getLayout().getImageRegion());
					slide.add(media);
					break;
				}
				if (type == VIDEO && !slide.hasVideo() && !slide.hasImage() && !slide.hasAudio() 
				        && !slide.hasVcard() && !slide.hasOtherFile()) {
					isSlideAdd = true;
					media = new VideoModel(mContext, dataUri, mSlideshow
							.getLayout().getImageRegion());
					slide.add(media);
					slide.updateDuration(media.getDuration());
					break;
				}
				if (type == AUDIO && !slide.hasAudio() && !slide.hasVideo() && !slide.hasVcard()
				        && !slide.hasOtherFile()) {
					isSlideAdd = true;
					media = new AudioModel(mContext, dataUri);
					slide.add(media);
					slide.updateDuration(media.getDuration());
					break;
				}
			...
			if(!isSlideAdd){
				SlideshowEditor slideShowEditor = new SlideshowEditor(mContext,
						mSlideshow);
				if (slideShowEditor.addNewSlide()) {
					SlideModel slide = mSlideshow.get(mSlideshow.size() - 1);
					if (type == IMAGE) {
			            ...
			            String src = (String)mContext.getResources().getText(R.string.attach_vcard);
			            media = new VcardModel(mActivity, ContentType.TEXT_VCARD, src, dataUri);
                    }  else if (type == OTHER_FILE) {
                        String uriStr = dataUri.toString();
                        String[] str = uriStr.split("/");
                        int num = str.length > 0 ? (str.length - 1) : 0;

                        media = new FileModel(mContext, ContentType.FILE_UNSPECIFIED,
                                ContentType.FILE_UNSPECIFIED, str[num], dataUri);
                    } else {
						throw new IllegalArgumentException("changeMedia type="
								+ type + ", uri=" + dataUri);
					}
					slide.add(media);
					if (type == VIDEO || type == AUDIO) {
        ...
        if (result == OK) {
            mAttachmentType = SLIDESHOW;
            mStatusListener.onAttachmentChanged();
        } else if (!isSlideAdd && result != SLIDE_MAX) {
            // We added a new slide and what we attempted to insert on the slide failed.
            // Delete that slide, otherwise we could end up with a bunch of blank slides.
    ...
    // ======modify by phone2 at 2012-02-15 begin======fix for bug 10630
    private ContactList checkFDN(ContactList contactList) {
        ContactList finalContactList = new ContactList();
        for (Contact contact : contactList) {
            if (FDNUtils.getInstance().isFDNDisable(contact.getNumber())) {
                Toast.makeText(
                        mContext,
                        mContext.getString(R.string.send_failed_fdn_only,
    ...
    private ContactList checkFDN(ContactList contactList,int phoneID) {
        ContactList finalContactList = new ContactList();
        for (Contact contact : contactList) {
            if (FDNUtils.getInstance(phoneID).isFDNDisable(contact.getNumber())) {
                Toast.makeText(
                        mContext,
                        mContext.getString(R.string.send_failed_fdn_only,
                                contact.getName()), Toast.LENGTH_SHORT).show();
	...
	// ======fixed CR<NEWMS00120798> by luning at 2011.11.09 begin======
	public void addVcard(VcardModel vcardModel){
		ensureSlideshow();
		mSlideshow.addVcard(vcardModel);
		mSlideshow.increaseMessageSize(vcardModel.getMediaSize());
		mAttachmentType = VCARD;
		mStatusListener.onAttachmentChanged();	
		updateState(HAS_ATTACHMENT, hasAttachment(), true);
	...
	// ======fixed CR<NEWMS00120798> by luning at 12011.11.09 end======
    public int getSubjectLength() {
        return mSubjectLength;
    }
    public void initTextToSlideshow() {
        int result = OK;
        try{
	        //if (requiresMms())
	        {
