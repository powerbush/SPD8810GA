...
import com.android.mms.data.ContactContentObserber;
...
public class MmsApp extends Application {
    ...
    public void onCreate() {
        ...
        // fixed bug for 19102 19105 start ,move in ConversationList oncreate().
        ...
        if(sharedPreferences.contains("pref_key_sms_retry_times")){
            boolean smsRetryTimes = sharedPreferences.getBoolean("pref_key_sms_retry_times", true);
            if(!smsRetryTimes){
    ...
    public TelephonyManager getTelephonyManager(int phoneId) {
	return  (TelephonyManager)getApplicationContext()
	    .getSystemService(PhoneFactory.getServiceName(Context.TELEPHONY_SERVICE, phoneId));
    }

    public boolean hasAnyIccCard() {
	TelephonyManager tm=getTelephonyManager();
	int phoneCount=tm.getPhoneCount();
	for (int i=0;i<phoneCount;++i) {
	    if (getTelephonyManager(i).hasIccCard()) {
		return true;
	    } 
	}
	return false;
