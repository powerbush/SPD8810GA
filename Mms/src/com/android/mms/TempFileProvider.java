...
public class TempFileProvider extends ContentProvider {
    ...
    public String getType(Uri uri) {
        return ContentType.FILE_UNSPECIFIED;
    }
     ...
     */
    public static String getScrapPath(String fileName, Context context) {
        // fix bug 17195 may be NullPointer in this method.
        String cacheDir = "";
        if (context != null) {
            if ( MessageUtils.getSdcardStatus() == MessageUtils.SDCARD_WRITE ) {
                cacheDir = context.getExternalCacheDir().getAbsolutePath();
            } else {
                cacheDir = context.getCacheDir().getAbsolutePath();
            }
        } else {
            cacheDir = "/mnt/sdcard/Android/data/com.android.mms";
        }
        Log.d(TAG, "ScrapPath:"+cacheDir + "/" + fileName);
        return cacheDir + "/" + fileName;
    }

    public static String getScrapPath(Context context) {
        return getScrapPath(".temp.jpg",context);
    }
    ...
    public static Uri renameScrapFile(String fileExtension, String uniqueIdentifier,Context context) {
        ...
        //Add by huibin for rotate the Picture
