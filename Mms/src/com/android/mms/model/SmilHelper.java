...
public class SmilHelper {
    ...
    public static final int SMIL_PLAY_MINIMUM_TIME = 1000;/*fixed CR<NEWMS00144369> by luning at 2011.11.25*/
    ...
    private static SMILDocument validate(SMILDocument in) {
        ...
        NodeList seq_nodelist = in.getBody().getElementsByTagName("seq");
        if(seq_nodelist != null && seq_nodelist.getLength() > 0){
            //fix for bug 12289, nokia phone send the Mms with slide to 8810,open the mms received in 8810, then crash
            //8810 can't resolve the smil file of Mms,send by 8810.
            //check the <seq> element,then return null;
            return null; 
        }
        return in;
    }
    ...
    private static SMILDocument createSmilDocument(PduBody pb) {
        ...
        for (int i = 0; i < partsNum; i++) {
            ...
            if ((par == null)
                    || (hasMedia && hasText)
                    /*fixed CR<NEWMS00145628> by luning at 2011.11.30 begin*/
                    || ((contentType.equals(ContentType.TEXT_PLAIN)
                            || contentType.equalsIgnoreCase(ContentType.APP_WAP_XHTML) || contentType
                            .equals(ContentType.TEXT_HTML)) && hasText)) {
                    /*fixed CR<NEWMS00145628> by luning at 2011.11.30 end*/
                par = addPar(document);
                hasText = false;
                hasMedia = false;
            }
            ...
            if (contentType.equals(ContentType.TEXT_PLAIN)
                ...
                hasMedia = true;
            } else if (contentType.equals(ContentType.FILE_UNSPECIFIED)) {
                SMILMediaElement vcardElement = createMediaElement(
                        ELEMENT_TAG_FILE, document, part.generateLocation());
                par.appendChild(vcardElement);
                hasMedia = true;
    ...
    private static SMILDocument createSmilDocument(SlideshowModel slideshow) {
        ...
        Collections.copy(slides, slides);
            ...
            for (MediaModel media : slide) {
                ...
                if (media instanceof TextModel) {
                    ...
                    imgRegionPresentInLayout = setRegion((SMILRegionMediaElement) sme,
                                                        ...
                                                        LayoutModel.IMAGE_REGION_ID,
                                                        imgRegionPresentInLayout);
                }  else if (media instanceof FileModel) {
                    sme = SmilHelper.createMediaElement(SmilHelper.ELEMENT_TAG_FILE, document, src);
                    imgRegionPresentInLayout = setRegion((SMILRegionMediaElement) sme,
                                                        smilRegions,
                                                        layoutElement,
                                                        LayoutModel.IMAGE_REGION_ID,
