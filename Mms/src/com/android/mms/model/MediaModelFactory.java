...
public class MediaModelFactory {
    ...
    private static PduPart findPart(PduBody pb, String src) {
            ...
            if (src.startsWith("cid:")) {
                ...
                if (part == null) {
                    ...
                    if (part == null) {
                        ...
                        // Sometimes, we can't find the part by ContentLocation, one of
                        // the reasons is that maybe src in smil xml contains type,like <text_0.txt>,
                        // but the ContentLocation in database does not. So we should still try to
                        // match media file name the src contains.
                        // It concerns to different Operators.
                        if (part == null) {
                            part = pb.getPartByContentLocation(src.substring(0,src.indexOf(".")));
                        }
                    }
                }
            }
        ...
        if (part != null) {
            part.findBySmil = true;/* fixed CR<NEWMS00133151,NEWMS00144166> by luning at 2011.12.06 */
            return part;
        }
        ...
        if (ContentType.isDrmType(contentType)) {
            ...
            if (tag.equals(SmilHelper.ELEMENT_TAG_TEXT)) {
                ...
                media = new VcardModel(context, contentType, src,
                        wrapper);
            } else if (tag.equals(SmilHelper.ELEMENT_TAG_FILE)) {
                media = new FileModel(context, contentType, src,
                        wrapper);
                ...
                if (ContentType.isTextType(drmContentType)) {
                    ...
                    media = new AudioModel(context, contentType, src,
                            wrapper);
                } else if (drmContentType.equals(ContentType.TEXT_VCARD)) {
                    media = new VcardModel(context, contentType, src,
                            wrapper);
                }  else if (drmContentType.equals(ContentType.FILE_UNSPECIFIED)) {
                    media = new FileModel(context, contentType, src,
                            wrapper);
                } else {
...
//                        part.getData());
                media = new VcardModel(context, contentType, src,
                      part.getDataUri());//fix for bug 10852
            } else if (tag.equals(SmilHelper.ELEMENT_TAG_FILE)) {
                media = new FileModel(context, contentType, src, part.getDataUri());
            } else if (tag.equals(SmilHelper.ELEMENT_TAG_REF)) {
            ...
            if ((tl != null) && (tl.getLength() > 0)) {
                ...
                if (t.getTimeType() != Time.SMIL_TIME_INDEFINITE) {
                    ...
                    if (duration == 0 &&
						...
						duration = media.initMediaDuration();
						if (duration == 0) {
