...
public class SlideshowPresenter extends Presenter {
        ...
        try {
            for (MediaModel media : model) {
                if (media instanceof RegionMediaModel) {
                    ...
                    presentFile(view, (FileModel) media, true);
                }
            }
    ...
    protected void presentVcard(SlideViewInterface view, VcardModel r, boolean dataChanged) {

//        view.setText(mContext.getString(R.string.slide_show_vcard),mContext.getString(R.string.slide_show_vcard));
//        view.setTextVisibility(true);
        view.setVcard(r.getUri(), r.getSrc());
    }

    protected void presentFile(SlideViewInterface view, FileModel r, boolean dataChanged) {
      view.setFile(r.getUri(), r.getSrc());
    }
    ...
    protected void presentImage(SlideViewInterface view, ImageModel image,
        ...
        if (dataChanged) {
        	...
        	if(image.getContentType().equals(ContentType.IMAGE_GIF)){
				view.setImage(image.getUri(), image.getBitmapWithDrmCheck(), true);
        	}else{   		
    ...
    public void onModelChanged(final Model model, final boolean dataChanged) {
        ...
        if (model instanceof SlideshowModel) {
            ...
            } else if (((VcardModel) model).isVcard()) {
                mHandler.post(new Runnable() {
                    public void run() {
