...
public class SlideView extends AbsoluteLayout implements
        AdaptableSlideViewInterface ,GifDecodeInterface{
	...
	public void initHandler(){
		...
		if (null == notifyShowGif) {
			notifyShowGif = new Handler(){
				public void handleMessage(Message msg) {
					if (msg.what == 101) {//success
						nextFrame();
					}else if(msg.what == -101){//fail
						recycleGif();
    ...
    {
		if (null != notifyShowGif && null != gifHandler) {
		    Log.d(TAG, "nextFrame");
			mImageView.setImageBitmap(gifHandler.nextFrameBitmap());
			notifyShowGif.postDelayed(nextFrame, gifHandler.getDelay());
		}		   	
    } 
    ...
    {	
    	if(null != notifyShowGif)
    	{    		
    		notifyShowGif.removeCallbacks(nextFrame);
    	}
    	if(null != gifHandler){
    ...
    public void setImage(String name, Bitmap bitmap) {
    	// ======fixed CR<NEWMS00127016> by luning at 11-09-28 begin======
    	recycleGif();
    	// ======fixed CR<NEWMS00127016> by luning at 11-09-28 end======
        if (mImageView == null) {
    ...
    // ======fixed CR<NEWMS00110179> by luning at 11-08-12 begin======
    //Slide show gif data need more time to prepare,we should show progress to user.
	public void setImage(final Uri uri, final Bitmap bitmap, boolean isGifImage) {
		setImage(null, bitmap);//show first frame at once
		if (null != uri) {
			initHandler();//init handler
			
			/*modify by luning at 2011.12.20 begin*/
//            new Thread(new Runnable() {
//                public void run() {//use a new thread to show gif data
//                    try {
//                        InputStream data = mContext.getContentResolver().openInputStream(uri);
//                        gifHandler.initGifData(data);//init gif data
//                        notifyShowGif.sendEmptyMessage(101);//notify show gif image
//                    } catch (OutOfMemoryError error) {
//                        notifyShowGif.sendEmptyMessage(-101);//recyle gif data
//                    } catch (Exception exception) {
//                        notifyShowGif.sendEmptyMessage(-101);
//                    }
//                }
//            }).start();
            ...
            new Thread(new Runnable() {
                public void run() {//use a new thread to show gif data
                    try {
                        InputStream data = mContext.getContentResolver().openInputStream(uri);
                        if (null != gifHandler.initGifData(data)) {
                            notifyShowGif.sendEmptyMessage(101);
                        } 
                    } catch (OutOfMemoryError error) {
                        if (null != bitmap) {
                            //we do nothing here,because of we have show first frame at once.
                        } else {
                            notifyShowGif.sendEmptyMessage(-101);
                        }
                    } catch (Exception exception) {
...
//	
//	private SmilPlayer mPlayer;
//
//	public void setSmilPlayer(SmilPlayer player) {
//		mPlayer = player;
//	}
//	
//	public void slideWait() {
//		mScanningDialog = ProgressDialog.show(mContext, null, mContext
//				.getResources().getString(R.string.wait_message), true, true);
//		if (mPlayer != null) {
//			mPlayer.pause();
//		}
//	}
...
//	}
    ...
    private void initAudioInfoView(String name) {
        ...
        // ======fixed CR<NEWMS00117791> by luning at 11-08-25 begin======
        if(null != mAudioInfoView){
        	TextView audioName = (TextView) mAudioInfoView.findViewById(R.id.name);
            audioName.setText(name);
        }
        // ======fixed CR<NEWMS00117791> by luning at 11-08-25 end======
    ...
    synchronized public void setAudio(Uri audio, String name, Map<String, ?> extras) {
        if (audio == null) {
    ...
    synchronized public void seekAudio(int seekTo) {
        if ((mAudioPlayer != null) && mIsPrepared) {
    ...
    public void setVcard(Uri uri, String name) {
        ...
        setText(name, getResources().getString(R.string.slide_show_vcard));
    }
