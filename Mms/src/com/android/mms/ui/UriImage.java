...
public class UriImage {
    ...
    private void initFromContentUri(Context context, Uri uri) {
        ...
        try {
            ...
            if (ImageModel.isMmsUri(uri)) {
                ...
                if (mContentType.contains("image")) {
                    mOrientation = c.getInt(c.getColumnIndexOrThrow(Images.Media.ORIENTATION));
                }

                Log.d(TAG, "initFromContentUri image orientation:" + mOrientation);
            }
            mPath = filePath;
        } finally {
            c.close();
        ...
        try {
            ...
            do {
                ...
                try {
                    ...
                    if (options.outWidth > widthLimit || options.outHeight > heightLimit || mOrientation != 0) {
                        ...
                        if (mContentType.contains("image")) {
                            ...
                            b = Bitmap.createScaledBitmap(b, outWidth / scaleFactor, outHeight
                                    / scaleFactor, false);
                        }
    ...
    private Bitmap rotateAndScale(Bitmap b, int degrees, int dstWidth, int dstHeight, boolean filter) {
        if (b != null) {
            Matrix m = new Matrix();
            ...
            // Log.d(TAG, "degree="+degrees+";sx="+sx+";sy="+sy);

            try {
                Bitmap b2 = Bitmap.createBitmap(b, 0, 0, width, height, m, filter);
                if (b != b2) {
                    b.recycle();
                    b = b2;
                }
