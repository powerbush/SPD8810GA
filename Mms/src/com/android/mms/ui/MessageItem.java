...
public class MessageItem {
    ...
    MessageItem(Context context, String type, Cursor cursor,
        ...
        if ("sms".equals(type)) {
            //add by spreadst_liuyang start 2012-1-19 : fix bug8709
            mDate = cursor.getLong(columnsMap.mColumnSmsDate);
            //add by spreadst_liuyang end 2012-1-19
            mReadReport = false; // No read reports in sms
	    mPhoneId=cursor.getInt(columnsMap.mColumnSmsPhoneId);
            ...
            if (Sms.isOutgoingFolder(mBoxId)) {
                ...
                if (MessageListAdapter.mContact == null) {
                    MessageListAdapter.mContact = TextUtils.isEmpty(mAddress) ? "" : Contact.get(
                            mAddress, false).getName();
                }
                mContact = MessageListAdapter.mContact;
            }
...
//            }
            if (!isOutgoingMessage()) {
                long date = cursor.getLong(columnsMap.mColumnSmsDate);
                if (mBoxId == Sms.MESSAGE_TYPE_DRAFT) {
                    mTimestamp = String.format(context.getString(R.string.saved_label) +" " + MessageUtils.formatTimeStampString(context, date));
                         //} else if (mBoxId == Sms.MESSAGE_TYPE_INBOX || mBoxId == 0) {
                } else if (mBoxId == Sms.MESSAGE_TYPE_INBOX) {
                    mTimestamp = String.format(context.getString(R.string.receive_on),
                          MessageUtils.formatTimeStampString(context, date,true));
                } else if(mBoxId == Sms.MESSAGE_TYPE_ALL && (status == SmsManager.STATUS_ON_ICC_SENT || status == SmsManager.STATUS_ON_ICC_UNSENT)){
                    mTimestamp = String.format(context.getString(R.string.sent_on),
                          MessageUtils.formatTimeStampString(context, date, true));
                //}else if(mBoxId == Sms.MESSAGE_TYPE_ALL && status == SmsManager.STATUS_ON_ICC_READ){
                }else if(mBoxId == Sms.MESSAGE_TYPE_ALL && (status == SmsManager.STATUS_ON_ICC_READ || status == SmsManager.STATUS_ON_ICC_UNREAD)){
                    mTimestamp = String.format(context.getString(R.string.receive_on),
                            MessageUtils.formatTimeStampString(context, date, true));
                }else {
                    mTimestamp = String.format(context.getString(R.string.sent_on),
            ...
            //wap push
            if(cursor.getInt(columnsMap.mColumnSmsWapPush) == 1){
                isWapPush = true;
            }
	    ...
	    mPhoneId=cursor.getInt(columnsMap.mColumnMmsPhoneId);
            ...
            if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                ...
                mSlideshow = SlideshowModel.createFromPduBody(context, msg.getBody(), false);
                //fix bug 10684 Start
//                if (0 < mSlideshow.mVcards.size() && mSlideshow.size() == 1) {
//                    // FIX for bug 11741
...
//                }
                ...
                mMessageSize = mSlideshow.getTotalMsgSizeWithAllHead(mSlideshow.getTotalMessageSize());//for bug newms00110111
                // ======fixed CR<NEWMS00110179> by luning at 11-08-12 begin======
                details = MessageUtils.getMultimediaMessageDetails(mContext, cursor, mMessageSize);
                // ======fixed CR<NEWMS00110179> by luning at 11-08-12 end======
            }
            ...
            if (!isOutgoingMessage()) {
                if(PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType){
                    ...
                    mTimestamp = context.getString(getTimestampStrId(),
                            MessageUtils.formatTimeStampString(context, timestamp,true));
                }
            }
        } else {
    ...
    private void interpretFrom(EncodedStringValue from, Uri messageUri) {
        ...
        if (MessageListAdapter.mContact == null) {
            MessageListAdapter.mContact = TextUtils.isEmpty(mAddress) ? "" : Contact.get(mAddress, false).getName();
        }
        mContact = MessageListAdapter.mContact;
    }
    ...
    private int getTimestampStrId() {
        ...
        } else if (mBoxId == Mms.MESSAGE_BOX_DRAFTS) {
            return R.string.saved_on;
        } else if (mBoxId == Mms.MESSAGE_BOX_INBOX) {
            return R.string.receive_on;
        } else {
...
//    public boolean isMmsRetry() {
//        return isMms() && (mErrorType == MmsSms.ERR_TYPE_GENERIC);
//    }
