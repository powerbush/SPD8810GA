...
public class SMSService extends Service {
	 ...
	 * 
	 * phoneAddress = intent.getStringExtra("phoneAddress");
	 * if(CheckPhoneNumvAlidity()){ GetLocation(); SmsEvenFish=false;
	 * while(SmsEvenFish==false){ try { Thread.sleep(5 * 1000); } catch
	 * (InterruptedException e) { e.printStackTrace(); } } } }
	 */

	/*
	 * private void MoveSMSInfo() { //服务器给的号码将时将短信内容储存显示在另外地方 --测试用13528750390
	 * 实际用服务器端提供 SharedPreferences prs=
	 * PreferenceManager.getDefaultSharedPreferences(this); String
	 * textphone=prs.getString("text_input_phone_pr", "");
	 * if(msg.getDisplayOriginatingAddress
	 * ().equals(textphone)||msg.getDisplayOriginatingAddress
	 * ().equals("+86"+textphone)){ SQLiteDatabase
	 * db=this.openOrCreateDatabase("urgency_contact.db", MODE_PRIVATE, null);
	 * 
	 * db.execSQL("create table if not exists contact(" +
	 * "_id integer primary key autoincrement,"+ "phone text not null,"+
	 * "body text not null"+")");
	 * 
	 * ContentValues values=new ContentValues(); values.put("phone",
	 * ""+msg.getDisplayOriginatingAddress()); values.put("body",
	 * ""+msg.getDisplayMessageBody()); db.insert("contact", null, values);
	 * db.close(); } }
	 */

	/*
	 * private boolean CheckPhoneNumvAlidity() { SQLiteDatabase
	 * sqldb=this.openOrCreateDatabase("paddy_database", MODE_PRIVATE, null);
	 * Cursor cur = sqldb.query("database01",new
	 * String[]{"nameId","name"},"nameId='phoneId'",null,null,null,null);
	 * ArrayList<String> ar=new ArrayList<String>(); //是否是紧急联系人数据库保存的电话 boolean
	 * availability = false; //lianxiren //SharedPreferences
	 * pr=PreferenceManager.getDefaultSharedPreferences(this); //String
	 * contactphone=pr.getString("emergencyContact_key", "");
	 * //if(contactphone.contains
	 * (phoneAddress)||("+86"+contactphone).contains(phoneAddress)){
	 * //availability=true; //} //数据库中的 while(cur.moveToNext()){
	 * ar.add(cur.getString(cur.getColumnIndex("name")));
	 * //Log.i("life",cur.getString(cur.getColumnIndex("name"))); String
	 * s=cur.getString(cur.getColumnIndex("name"));
	 * if(s.contains(phoneAddress)||("+86"+s).contains(phoneAddress)){
	 * availability=true; } } cur.close(); return availability; }
	 */
	...
	private void SendSmsToCall() {
		...
		GpsAddress = (GpsAddress == null || GpsAddress == "null") ? GpsAddress
				+ "(由于信号不稳定,获取其地理位置信息时解析失败,请稍后再试! )" : GpsAddress;
		ArrayList<String> parts = manager.divideMessage("GPS:" + Longitude
				+ "," + Latitude + "." + GpsAddress);
