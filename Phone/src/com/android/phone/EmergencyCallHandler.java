...
public class EmergencyCallHandler extends Activity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        int phoneId = getIntent().getIntExtra(Phone.PHONE_ID, 0);
        if (DBG) Log.d(TAG, "phoneId :" + phoneId);
        Phone phone = PhoneApp.getInstance().getPhone(phoneId);
        ...
        if (retryCount == INITIAL_ATTEMPT) {
            boolean isNeedToAirplaneModeOff = eci.intent.getBooleanExtra(
                    "isNeedToAirplaneModeOff", false);
            // place the number of pending retries in the intent.
            if ((SystemProperties.getInt("persist.sys.emergencyCallRetry", 0) == 1)
                    && (!isNeedToAirplaneModeOff)) {
                if (DBG) Log.d(TAG, "persist.sys.emergencyCallRetry...1");
            ...
            if (Settings.System.getInt(getContentResolver(), 
                ...
                // only for double and sigle card,make a normal call. if there is multi cards,
                intent.putExtra("emergency", true);

                intent.putExtra("isNeedToAirplaneModeOff", isNeedToAirplaneModeOff);
                intent.putExtra(Phone.PHONE_ID, phoneId);

                sendBroadcast(intent);

                if (isNeedToAirplaneModeOff && PhoneApp.getInstance().getActiveSubCount() == 0) {
                    // for conditions that can not make a call,such as double
                    // forbidden, no card exist, and sigle card but forbidden,
                    eci.phone.unregisterForServiceStateChanged(sHandler);
                    eci.dialog.dismiss();
                    if (DBG)
                        Log.d(TAG, "double forbidden. can't make a normal call . " + eci.intent);

                    eci.intent.putExtra("isNeedToAirplaneModeOff", false);
                    eci.app.startActivity(eci.intent);
                    finish();
                    return;
                }
            ...
            // we just turn it back on.
                ...
                Settings.System.putInt(getContentResolver(), PhoneFactory.getSetting(
                        Settings.System.SIM_STANDBY, phoneId), 1);
            }
    ...
    private ProgressDialog constructDialog(int retryCount) {
        ...
        if (DBG) Log.d(TAG, "ProgressDialog text:" + getText(msgId));
