...
public class MenuHelper {
    ...
    //fixed bug 16734
    public static AlertDialog confirmAction(Context context, String title,
    ...
    private static void gotoGallery(Activity activity, int windowTitleId,
        ...
        Uri target =
            Images.Media.EXTERNAL_CONTENT_URI.buildUpon()
                .appendQueryParameter(
                    "bucketId", ImageManager.getCameraImageBucketId())
                        .build();
    ...
    public static int calculatePicturesRemaining() {
        try {
            if (!ImageManager.hasStorage()) {
                ...
                float remain = (((float) blocks) * ((float) block_size) / PICTURE_BYTES);
                Log.d(TAG,
                    String.format("calculate picture remain blocks == %d --- size == %d, remain == %f",
