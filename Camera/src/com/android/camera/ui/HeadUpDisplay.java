...
public class HeadUpDisplay extends GLView {
    ...
    private static final float MAX_WIDTH_RATIO = 0.95f;
    ...
    protected void onLayout(
        ...
        //fixed bug 16768   [0 update to top]
        mIndicatorBar.layout(
                width - mIndicatorBar.getMeasuredWidth() - rightMargin, top,
    ...
    private void hidePopupWindow() {
        if(mPopupWindow == null || mPopupWindow.getVisibility() == INVISIBLE){
            return;
        }
    ...
    private class IndicatorBarListener
        ...
        public void onItemSelected(GLView view, int position) {
            Log.v(TAG, "onItemSelected: view = " + view + " , position = " + position);
            AbstractIndicator indicator = (AbstractIndicator) view;
            if (mPopupWindow == null) {
                initializePopupWindow(getGLRootView().getContext());
            }

            if((mEnabled || view instanceof ZoomIndicator) == false){
                hidePopupWindow();
                mIndicatorBar.setSelectedIndex(IndicatorBar.INDEX_NONE);
                mIndicatorBar.setActivated(false);
                return;
    ...
    public void restorePreferences(final Parameters param) {
        ...
        // before clear backup current camera id
        int backup_camera_id = 0;
        try {
            backup_camera_id =
                CameraSettings.readPreferredCameraId(mSharedPrefs);
        } catch (Exception e) {
        } finally {
            editor.clear();
            // rewrite camera id
            editor.putString(
                CameraSettings.KEY_CAMERA_ID,
                    Integer.toString(backup_camera_id));
            editor.apply();
        }
