...
public class PhoneDisambigDialog implements DialogInterface.OnClickListener,
    ...
    private ContactsListActivity.Action mAction;
    ...
    public PhoneDisambigDialog(Context context, Cursor phonesCursor) {
        this(context, phonesCursor, false, 0, ContactsListActivity.Action.VOICECALL /*make call*/);
    }

//    public PhoneDisambigDialog(Context context, Cursor phonesCursor, ContactsListActivity.Action action) {

    public PhoneDisambigDialog(Context context, Cursor phonesCursor, boolean sendSms,
            int stickyTab, ContactsListActivity.Action action) {

        mContext = context;
        //mAction = ContactsListActivity.Action.VOICECALL;
        mAction = action;
        ...
        mPhonesAdapter = new PhonesAdapter(mContext, mPhoneItemList, mAction);
        ...
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(mContext).
                setAdapter(mPhonesAdapter, this).
                        setTitle((mAction == ContactsListActivity.Action.SENDSMS) ?
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (mPhoneItemList.size() > which && which >= 0) {
            ...
            if (mAction == ContactsListActivity.Action.SENDSMS) {
                ContactsUtils.initiateSms(mContext, phone);

            } else if (mAction == ContactsListActivity.Action.VOICECALL){

                ContactsUtils.initiateCall(mContext, phone);
            } else if (mAction == ContactsListActivity.Action.VIDEOCALL){
                ContactsUtils.initiateVideoCall(mContext, phone);
            }else {
                //google-2.3.5_r1
    ...
    private static class PhonesAdapter extends ArrayAdapter<PhoneItem> {
        private final ContactsListActivity.Action action;
        private final Sources mSources;

        public PhonesAdapter(Context context, List<PhoneItem> objects, ContactsListActivity.Action action) {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (kind != null) {
                ...
                StringInflater header = (action == ContactsListActivity.Action.SENDSMS) ? kind.actionAltHeader : kind.actionHeader;
