...
public class ContactPhotoLoader implements Callback {
  ...
  //lino modify begin for NEWMS00141500
    private final String []DATACOLUMNS = new String[] {Photo._ID,Photo.PHOTO,Data.MIMETYPE,Data.DATA_VERSION};
  //lino modify begin for NEWMS00141500
    /**
    ...
    private AsyncQueryHandler myHander; //modify by dory.zheng for NEWMS144047
    /**
     * Constructor.
     *
     ...
     */
    private void cacheBitmap(long id, byte[] bytes,int version) {
        if (mPaused) {
            return;
        }
...
//                        COLUMNS
                		DATACOLUMNS ////lino modify begin for NEWMS00141500
                        ,
                ...
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        ...
                        int version = cursor.getInt(cursor.getColumnIndex(Data.DATA_VERSION));//lino modify begin for NEWMS00141500
                        byte[] bytes = cursor.getBlob(1);
                        cacheBitmap(id, bytes,version);////lino modify begin for NEWMS00141500
                        mPhotoIds.remove(id);
                    }
                }
            } finally {
            ...
            for (int i = 0; i < count; i++) {
                cacheBitmap(mPhotoIds.get(i), null,0);////lino modify begin for NEWMS00141500
            }
        }
    }
}
