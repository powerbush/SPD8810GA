...
import com.android.contacts.util.VCardEntryApplyBatchHandler;
...
public class ImportVCardActivity extends Activity {
    ...
    //add by liguxiang 11-25-11 for NEWMS00140264 begin
    private static boolean  mImporting = false;
    private static int mLastCallState = 0;
    //add by liguxiang 11-25-11 for NEWMS00140264 end
    ...
    private class VCardReadThread extends Thread
        ...
        @Override
        public void run() {
            Log.v(LOG_TAG, "VCardReadThread:run()");
            ...
            if (mSelectedVCardFileList != null && mUri == null && mSelectedVCardFileList.size() == 1) {
                VCardFile vcardFile = mSelectedVCardFileList.get(0);
                mUri = Uri.parse("file://" + vcardFile.getCanonicalPath());
                mSelectedVCardFileList = null;
            }
            try {
                if (mUri != null) {  // Read one vCard expressed by mUri
                    Log.v(LOG_TAG, "VCardReadThread:mUri != null");
                    ...
                    createdUri = doActuallyReadOneVCard(targetUri, mAccount, true, counter.getCount(),
                            detector, mErrorFileNameList);
                    Log.v(LOG_TAG, "VCardReadThread:createdUri-->" + createdUri);
                    ...
                    int totalFileCount = mSelectedVCardFileList.size();
                    ...
                    for (VCardFile vcardFile : mSelectedVCardFileList) {
                        ...
                        VCardInterpreterCollection builderCollection =
                                new VCardInterpreterCollection(Arrays.asList(counter, detector));
                        try {
                        ...
                        createdUri = doActuallyReadOneVCard(targetUri, mAccount,
                                false, counter.getCount(), detector, mErrorFileNameList);
                        if (null != createdUri) {
                            successFileCount++;
                        }
                        if (mCanceled) {
                            String message = getString(R.string.import_aborted);
                    ...
                    message.append(successFileCount)
                    .append(" ")
                    .append(getString(R.string.import_vcard_files_flags))
                    .append(getString(R.string.label_success))
                    .append(", ")
                    .append((totalFileCount - successFileCount))
                    .append(" ")
                    .append(getString(R.string.import_vcard_files_flags))
                    .append(getString(R.string.label_fail));
        ...
        private Uri doActuallyReadOneVCard(Uri uri, Account account,
                boolean showEntryParseProgress,int entryCount,
                VCardSourceDetector detector, List<String> errorFileNameList) {
            Log.v(LOG_TAG, "doActuallyReadOneVCard");
            ...
            final VCardEntryApplyBatchHandler committer = new VCardEntryApplyBatchHandler(mResolver, entryCount);
            ...
            builder.addEntryHandler(new ProgressShower(mProgressDialogForReadVCard,
                    ...
                    mHandler, showEntryParseProgress, entryCount));
            ...
            ArrayList<Uri> createdUris = committer.getCreatedUris();
            final int totalCount = entryCount;
            final int successCount = createdUris.size();

            if (mSelectedVCardFileList == null) {
                StringBuffer message = new StringBuffer();
                message.append(" ")
                .append(successCount)
                .append(" ")
                .append(getString(R.string.label_success))
                .append(", ")
                .append((totalCount - successCount))
                .append(" ")
                .append(getString(R.string.label_fail));
...
//                mProgressDialogForReadVCard.dismiss();
                ...
                Log.d(LOG_TAG, "SQLiteDiskIOException message: " + e.toString());
    ...
    private class VCardScanThread extends Thread implements OnCancelListener, OnClickListener {
        ...
        public void run() {
            ...
            try {
                mWakeLock.acquire();
                Log.v(LOG_TAG, "mRootDirectory:"+mRootDirectory.getPath());
                getVCardFileRecursively(mRootDirectory);//recursively get the vcard file in mAllVCardFileList
                mCheckedPaths.clear();
                mCheckedPaths = null;
        ...
        private void getVCardFileRecursively(File directory)
            ...
            for (int i = 0; i < len; i++) {
                ...
                File file = files[i];
                String canonicalPath = file.getCanonicalPath();//the absolute path of this file
    ...
    private void startVCardSelectAndImport() {
        Log.v(LOG_TAG, "startVCardSelectAndImport");
        int size = mAllVCardFileList.size();
        if (getResources().getBoolean(R.bool.config_import_all_vcard_from_sdcard_automatically)) {
            importMultipleVCardFromSDCard(mAllVCardFileList);
        } else if (size == 1) {//只有一条记录
            runOnUIThread(new DialogDisplayer(R.id.dialog_import_one_vcard));
    ...
    private void importMultipleVCardFromSDCard(List<VCardFile> selectedVCardFileList) {
        final List<VCardFile> allVCardFileList = selectedVCardFileList;
        selectedVCardFileList = null;
        runOnUIThread(new Runnable() {
            public void run() {
                mVCardReadThread = new VCardReadThread(allVCardFileList);
    ...
    private void importOneVCardFromSDCard(final Uri uri) {
        Log.v(LOG_TAG, "importOneVCardFromSDCard");
        runOnUIThread(new Runnable() {
            public void run() {
    ...
    private Dialog importVcardWarn() {
        return new AlertDialog.Builder(this).setTitle(R.string.import_contacts)
                .setMessage(R.string.whether_import_from_SD)
                .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {

                    public void onClick(DialogInterface dialog, int which) {
                        String canonicalPath = mAllVCardFileList.get(0).getCanonicalPath();
                        Uri uri = Uri.parse("file://" + canonicalPath);
                        Log.v(LOG_TAG, "startVCardSelectAndImport:uri-->" + uri.toString());
                        importOneVCardFromSDCard(uri);
                    }
                }).setOnKeyListener(mOnkeyListener).setOnCancelListener(mCancelListener)
                .setNegativeButton(R.string.cancel, mCancelListener).create();
    }
    ...
    protected Dialog onCreateDialog(int resId) {
    	...
    	mImporting = true;  //add by liguxiang 11-25-11 for NEWMS00140264
        switch (resId) {
            ...
            case R.id.dialog_import_one_vcard:{
                return  importVcardWarn();
            }
            case R.id.dialog_reading_vcard: {
                if (mProgressDialogForReadVCard == null) {
                    ...
                    mProgressDialogForReadVCard.setOnKeyListener(mOnkeyListener);
...
//        //mImporting = false;
    ...
    protected void onDestroy() {
        ...
        mImporting = false; //modify by dory.zheng for import aborted display to call phone in import complete
    ...
    private void doScanExternalStorageAndImportVCard() {
        Log.v(LOG_TAG, "doScanExternalStorageAndImportVCard");
...
//        }
    ...
    /**
     * release var mAllVCardFileList resource
     */
    private void releaseAllVCardFileList() {
        if (null != mAllVCardFileList) {
            mAllVCardFileList.clear();
            mAllVCardFileList = null;
        }
    }
}
