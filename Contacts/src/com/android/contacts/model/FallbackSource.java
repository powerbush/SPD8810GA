...
public class FallbackSource extends ContactsSource {
	...
	private static final String TAG = "FallbackSource";
    ...
    // add date flag
    protected static final int FLAGS_DATE = EditorInfo.TYPE_CLASS_TEXT | EditorInfo.TYPE_CLASS_DATETIME;
    ...
    protected void inflate(Context context, int inflateLevel) {
        ...
        inflateBirthday(context, inflateLevel);
    ...
    protected DataKind inflateIm(Context context, int inflateLevel) {
        ...
        if (inflateLevel >= ContactsSource.LEVEL_CONSTRAINTS) {
            ...
            kind.typeList.add(buildImType(Im.PROTOCOL_OTHER));
    ...
    // keep it, maybe will add multiple event
    protected DataKind inflateEvent(Context context, int inflateLevel) {
        ...
        if (kind == null) {
            kind = addKind(new DataKind(Event.CONTENT_ITEM_TYPE,
                    R.string.eventLabelsGroup, -1, 130, true));
        ...
        if (inflateLevel >= ContactsSource.LEVEL_CONSTRAINTS) {
            ...
            kind.typeList = Lists.newArrayList();
            kind.typeList.add(buildEventType(Event.TYPE_BIRTHDAY).setSpecificMax(1));

            kind.fieldList = Lists.newArrayList();
            kind.fieldList.add(new EditField(Event.DATA, R.string.eventLabelsGroup, FLAGS_DATE));
        }

        return kind;
    }

    // just for add birthday event
    protected DataKind inflateBirthday(Context context, int inflateLevel) {
        DataKind kind = getKindForMimetype(Event.CONTENT_ITEM_TYPE);
        if (kind == null) {
            kind = addKind(new DataKind(Event.CONTENT_ITEM_TYPE,
                    R.string.label_birthday, -1, 20, true));
            kind.isList = false;
            kind.secondary = true;
            kind.actionHeader = new SimpleInflater(R.string.label_birthday);
            kind.actionBody = new SimpleInflater(Event.START_DATE);
        }
        ...
        if (inflateLevel >= ContactsSource.LEVEL_CONSTRAINTS) {
            ...
            kind.fieldList = Lists.newArrayList();
            kind.fieldList.add(new EditField(Event.DATA, R.string.label_birthday, FLAGS_DATE));
        }
    ...
    protected EditType buildEventType(int type) {
        EditType result = null;
        switch(type) {
            case Event.TYPE_BIRTHDAY :
                result = new EditType(type, R.string.label_birthday);
                break;
            default:
                break;
        }
    ...
    public static class ImActionInflater extends CommonInflater {
        ...
        protected int getTypeLabelResource(Integer type) {
            ...
            switch (type) {
                ...
                case Im.PROTOCOL_OTHER: return R.string.chat_other;
