...
public class ContactsUtils {
	...
	public static Intent getVPIntent(Intent intent){
		//lino modify for huawei KP52678 when call contacts but use video call 2011-11-09
		Intent newIntent = new Intent(intent);
//		return intent.putExtra(EXTRA_IS_VIDEOCALL, true);
		return newIntent.putExtra(EXTRA_IS_VIDEOCALL, true);
		//lino modify for huawei KP52678 when call contacts but use video call 2011-11-09
	}
    /**
     * Kick off an intent to initiate a video call.
     *
     * @param phoneNumber must not be null.
     * @throws NullPointerException when the given argument is null.
     */
    public static void initiateVideoCall(Context context, CharSequence phoneNumber) {
        Intent intent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
                Uri.fromParts("tel", phoneNumber.toString(), null));
        context.startActivity(getVPIntent(intent));
    }
    ...
    /**
     * add by dory.zheng for find by number is simCard number
     * @param id
     * @return
     */
    public static int isSimNumber(Context context, long id){
        Uri uri = Uri.parse("content://com.android.contacts/raw_contacts");
        Cursor cursor = null;
        int sim_index = 0;
        try {
            cursor = context.getContentResolver().query(uri, new String[]{"sim_index"}, "_id = ?", new String[]{String.valueOf(id)}, null);
            if(cursor.moveToFirst()){
                sim_index = cursor.getInt(0);
            }
        } finally{
            if(cursor != null){
                cursor.close();
    ...
    /**
     * add by dory.zheng for find by name is simCard
     * @param id
     * @return
     */
    public static String getSimName(Context context, long id){
        Uri uri = Uri.parse("content://com.android.contacts/data");
        Cursor cursor = null;
        String name = null;
        try {
            cursor = context.getContentResolver().query(uri, new String[]{StructuredName.DATA1}, StructuredName.MIMETYPE + " = '" + StructuredName.CONTENT_ITEM_TYPE + "' and raw_contact_id = ? ", new String[]{String.valueOf(id)}, null);
            if(cursor.moveToFirst()){
    ...
    public static String pAndwToCommaAndSemicolon(String str){
        if(null != str){
            return  str.replace('P', ',').replace('W', ';').replace('p', ',').replace('w', ';');
        }else{
            return null;
        }
    }
