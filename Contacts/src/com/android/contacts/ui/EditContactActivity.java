...
public final class EditContactActivity extends Activity
    ...
    private static final File NOSDCARD_PHOTO_DIR = new File("/data/internal_memory/DCIM/Camera/");
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (Intent.ACTION_EDIT.equals(action) && !hasIncomingState) {
            ...
            doAddAction(getIntent().getExtras());
        }
        ...
        protected EntitySet doInBackground(EditContactActivity target, Intent... params) {
            ...
            if (ContactsContract.AUTHORITY.equals(authority)) {
                if (Contacts.CONTENT_ITEM_TYPE.equals(mimeType)) {
                    ...
                    mSelection = RawContacts.CONTACT_ID + "=" + contactId + " AND sim_index=0";
                }
    ...
    protected void bindEditors() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (null == values || !values.isVisible()) continue;
    ...
    public void onBackPressed() {
        if (!doSaveAction(SAVE_MODE_DEFAULT)) {
            finish();
        }
    }
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_delete:
                ...
                if(!SimInitReceiver.isDelete){
                    ...
                    Toast.makeText(EditContactActivity.this, R.string.deleting, Toast.LENGTH_SHORT).show();
                    return false;
                }
    ...
    private void displayContactsGroupDialog() {
        // Wrap our context to inflate list items using correct theme
        ...
        adapter.add(R.string.group_phone);
        adapter.add(R.string.group_sim);

        final DialogInterface.OnClickListener clickListener =
                new DialogInterface.OnClickListener() {
        	private final int MODE_INSERT = 2;
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                int group;
                final int resId = adapter.getItem(which);
                switch (resId) {
                    case R.string.group_phone: {
                    ...
                    case R.string.group_sim: {
                    	if(!((TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE))
                    	        .hasIccCard()){
                    		Toast.makeText(EditContactActivity.this, getString(R.string.sim_no_ready), Toast.LENGTH_SHORT).show();
                    		return;
                    	}
                    	Intent intent=new Intent(Intent.ACTION_EDIT);
                		intent.setClass(EditContactActivity.this, SimEditContactActivity.class);
    ...
    public static class PersistTask extends
        ...
        private String customGroupRingtone = null;
        ...
        protected Integer doInBackground(EditContactActivity target, EntitySet... params) {
            ...
            while (tries++ < PERSIST_TRIES) {
                try {
                    ...
                    int contactsGroupNameId = isContactsGroupNameSelected(target);
                    if(result == RESULT_SUCCESS || contactsGroupNameId != -1){
                		updateRawContactsGroupNameId(target,rawContactId,contactsGroupNameId);
                    }
...
//                     * bin.lai
//                     */
//                    Log.d("dory", "result=="+result);
//                    if(result == RESULT_SUCCESS){
//                    	int contactsGroupNameId = isContactsGroupNameSelected(target);
//                    	if (contactsGroupNameId != -1) {
//                    		updateRawContactsGroupNameId(target,rawContactId,contactsGroupNameId);
//						}
//                    }
                    ...
                    Log.w(TAG, "No space left on device");
         ...
         * @return
         */
        int isContactsGroupNameSelected(EditContactActivity target){
        	LinearLayout linearLayout = (LinearLayout)target.findViewById(R.id.editors);
        	//add by 钱剑波 2011-8-19 for 114329 begin
        	Button button = (Button)linearLayout.findViewById(R.id.contact_group_button);
        	String name = button.getText().toString();
        	//add by 钱剑波 2011-8-19 for 114329 end

        	Cursor cursor = target.getContentResolver().query(DIVIDED_GROUP_URI, null, "divided_name=?",
        			new String[]{name}, null);
        	if(cursor.moveToNext()){
                customGroupRingtone = cursor.getString(cursor.getColumnIndex("divided_ringtone"));
        		int contactsGroupNameId = cursor.getInt(cursor.getColumnIndexOrThrow(BaseColumns._ID));
        		cursor.close();
        		return contactsGroupNameId;
        	}else{
        		cursor.close();
        		return -1;
        	}
         ...
         * @param rawContactId
         */
        private void updateRawContactsGroupNameId(EditContactActivity target,long rawContactId,int contactsGroupNameId){
        	ContentValues values = new ContentValues();

        	values.put("divided_group_name_id", contactsGroupNameId);
            if (null != customGroupRingtone) {
                values.put("custom_group_ringtone", customGroupRingtone);
            }

        	target.getContentResolver().update(Uri.parse("content://"+ContactsContract.AUTHORITY+"/raw_contacts")
        			, values, BaseColumns._ID+"=?", new String[]{String.valueOf(rawContactId)});
        }
    ...
    boolean doSaveAction(int saveMode) {
        ...
        for(EntityDelta entityDelta:mState){
        	ArrayList<ValuesDelta> nameMimeEntries =entityDelta.getMimeEntries(StructuredName.CONTENT_ITEM_TYPE);
        	ArrayList<ValuesDelta> phoneMimeEntries =entityDelta.getMimeEntries(Phone.CONTENT_ITEM_TYPE);
            ArrayList<ValuesDelta> emailMimeEntries = entityDelta.getMimeEntries(Email.CONTENT_ITEM_TYPE);

        	if(nameMimeEntries!=null&&nameMimeEntries.size()!=0){
        		for(ValuesDelta value:nameMimeEntries){
        			if(value!=null){
        				familyName=value.getAsString(StructuredName.FAMILY_NAME);
                		givenName=value.getAsString(StructuredName.GIVEN_NAME);
                		if(familyName!=null&&!"".equalsIgnoreCase(familyName)
                			||givenName!=null&&!"".equalsIgnoreCase(givenName)){
                			break;
                		}
        			}
            	}
            ...
            if (phoneMimeEntries != null && phoneMimeEntries.size() != 0) {
                for (ValuesDelta value : phoneMimeEntries) {
                    if (value != null) {
                        String phoneNum = value.getAsString(Phone.NUMBER);
                        if (!TextUtils.isEmpty(phoneNum)) {
                            if(!Config.isMSMS){
			    if (!CommonUtil.isPhoneNumber(phoneNum)) {
                                Toast.makeText(this,
                                        R.string.contact_invalid_phone_number_error_toast,
        	...
        	if((familyName==null||"".equalsIgnoreCase(familyName))
        			&&(givenName==null||"".equalsIgnoreCase(givenName))
        			&&(!hasPhoneNum)){
        		Toast.makeText(this, R.string.toast_name_number_both_empty, Toast.LENGTH_SHORT).show();
        		return false;
        	}

            // add check for email address
            if (null != emailMimeEntries && emailMimeEntries.size() > 0) {
                for (ValuesDelta value : emailMimeEntries) {
                    if (null != value) {
                        String email = value.getAsString(Email.ADDRESS);
                        if (!TextUtils.isEmpty(email) && !CommonUtil.isEmailAddress(email)) {
...
//    }
    ...
    protected void doTakePhoto() {
        try {
            ...
            if (hasSDCard()) {
                ...
                NOSDCARD_PHOTO_DIR.mkdirs();
                mCurrentPhotoFile = new File(NOSDCARD_PHOTO_DIR, getPhotoFileName());
            }
    ...
    public int compare(EntityDelta one, EntityDelta two) {
        ...
        Log.i(TAG,"oneIsGoogle :"+oneIsGoogle +"twoIsGoogle :"+twoIsGoogle);
        ...
        Log.i(TAG,"oneSource.accountType :"+oneSource.accountType);
     ...
     * {@link EntityDelta} under the currently edited {@link Contacts}.
     */
    private boolean doAddAction(Bundle bundle) {
        if (mStatus != STATUS_EDITING) {
            return false;
        }
        ...
        if(null != bundle){
            accountName = bundle.getString(RawContacts.ACCOUNT_NAME);
